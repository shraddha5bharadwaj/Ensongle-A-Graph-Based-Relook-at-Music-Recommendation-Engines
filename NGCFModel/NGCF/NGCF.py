...
class NGCF(object):
    def __init__(self, data_config, pretrain_data):
        ...
        *********************************************************
        ...
        self.node_dropout = tf.placeholder(tf.float64, shape=[None])
        self.mess_dropout = tf.placeholder(tf.float64, shape=[None])
    ...
    def _init_weights(self):
        ...
        if True :  #Change to false  to use xavier initialisation
            # user_embed = np.load("user_feature_vector.npy", allow_pickle=True).astype(dtype="float64")
            user_embed = np.load("user_feature_vector_train.npy", allow_pickle=True).astype(dtype="float64")
            all_weights['user_embedding'] = tf.Variable(user_embed, name='user_embedding')
            # all_weights['user_embedding'] = tf.Variable(initializer([self.n_users, self.emb_dim], dtype=tf.float64), name='user_embedding')
            song_embed = np.load("song_feature_vector.npy", allow_pickle=True).astype(dtype="float64")
            all_weights['item_embedding'] = tf.Variable(song_embed, name='item_embedding')
            print("Loading custom feature embeddings")
            
        elif self.pretrain_data is None:
            all_weights['user_embedding'] = tf.Variable(initializer([self.n_users, self.emb_dim], dtype=tf.float64), name='user_embedding')
            all_weights['item_embedding'] = tf.Variable(initializer([self.n_items, self.emb_dim], dtype=tf.float64), name='item_embedding')
        ...
        for k in range(self.n_layers):
            all_weights['W_gc_%d' %k] = tf.Variable(
                initializer([self.weight_size_list[k], self.weight_size_list[k+1]], dtype=tf.float64), name='W_gc_%d' % k)
            all_weights['b_gc_%d' %k] = tf.Variable(
                initializer([1, self.weight_size_list[k+1]], dtype=tf.float64), name='b_gc_%d' % k)

            all_weights['W_bi_%d' % k] = tf.Variable(
                initializer([self.weight_size_list[k], self.weight_size_list[k + 1]], dtype=tf.float64), name='W_bi_%d' % k)
            all_weights['b_bi_%d' % k] = tf.Variable(
                initializer([1, self.weight_size_list[k + 1]], dtype=tf.float64), name='b_bi_%d' % k)

            all_weights['W_mlp_%d' % k] = tf.Variable(
                initializer([self.weight_size_list[k], self.weight_size_list[k+1]], dtype=tf.float64), name='W_mlp_%d' % k)
            all_weights['b_mlp_%d' % k] = tf.Variable(
                initializer([1, self.weight_size_list[k+1]], dtype=tf.float64), name='b_mlp_%d' % k)
    ...
    def create_bpr_loss(self, users, pos_items, neg_items):
        ...
        reg_loss = tf.constant(0.0, tf.float64, [1])
...
def load_pretrained_data():
    ...
    try:
        ...
        pretrain_data = None
    # pretrain_data={}
    # pretrain_data['user_embed'] = np.load("/content/drive/MyDrive/User_Profile/data/user_profile_vectors_only_numes.npy",allow_pickle=True)
    # pretrain_data['item_embed'] = X
...
if __name__ == '__main__':
    ...
    *********************************************************
    ...
    if args.epoch == -1:
        ...
        if os.path.isfile("rec_scores.npy"):
            os.remove("rec_scores.npy")
        if os.path.isfile("user_batch.txt"):
            os.remove("user_batch.txt")

        users_to_test = list(data_generator.train_items.keys())
        ret = test(sess, model, users_to_test , drop_flag=True, run_model=True)
        ...
        for epoch in range(args.epoch):
            ...
            for idx in tqdm(range(n_batch),ncols = 100):
        ...
        met_path="../saved_metrics/"
        
        # np.save(met_path+"recs.npy",recs)
        # np.save(met_path+"pres.npy",pres)
        # np.save(met_path+"ndcgs.npy",ndcgs)
        # np.save(met_path+"hit.npy",hit)
